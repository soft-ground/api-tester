...
//   key (columns = union of the elements' keys, first-seen order). This handles both a top-level
//   `data: [...]` and wrapped payloads like `{ code, data: { items: [...] } }`.
...
function tableFromObjects(arr: Record<string, unknown>[]): unknown[][] {
  const cols: string[] = [];
  for (const el of arr) {
    for (const k of Object.keys(el)) if (!cols.includes(k)) cols.push(k);
  }
  const rows: unknown[][] = [cols];
  for (const el of arr) rows.push(cols.map((c) => cell(el[c])));
  return rows;
}
